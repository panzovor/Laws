...
def filter(key,data):
    if len(data[key]) <min_samples_num:
        return False
    return True
...
### 每个类别的关键词比例（例如：借款人 丧失 偿还 能力，若句中出现其中3个词，则对应值为0.75）
        ...
        if len(self.keywords_parameter)>0:
            for key in self.keys:
                for w in self.keywords_parameter[key]:
                    if w in line:
                        count+=1
                result.append(count/len(self.keywords_parameter[key]))
        return result
            ...
            for key in keys:
                ...
                for w in words:
                    if w in self.tfidf_parameter[key].keys():
                        tmp.append(self.tfidf_parameter[key][w])
    ...
    def train(self,train_file):
        ...
        keywords_features(data)
        print("keywords train done")
        tfidf_features(data)
        print("tfidf train done")
        probability_features(data)
            ...
            for key in keys:
                ...
                for w in words:
                    if w in self.probability_parameter[key].keys():
                        tmp.append(self.probability_parameter[key][w])
    ...
    def get_features(self,line):
        ...
        # print(len(res),self.all_feature_len)
        #
        # print(len(self.keys),len(self.tfidf_parameter.keys())*self.tfidf_len_each,4,self.probabilit_len_each* len(self.probability_parameter.keys()))
        # print(len(key_fea),len(tfidf_fea),len(cha_fea),len(pro_fea))
