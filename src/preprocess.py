...
def __label_content1(content,labels):
    labeled_data = {}
    result = {}
    result_no_label ={}
    instrument_regex = "【--\d{1,6}--】"
    ...
    for lregex in sentence_label_regex:
        ...
        for i in range(len(asseys_no)):
            ...
            if len(fres) >0:
                for var in fres:
                    if len(var)>sentence_min_len:
                        result[lregex][no].append(var)
                        result_no_label[no][lregex].append(var)
                        labeled_data[lregex].append(var)
    ...
    for var in content.split("\n"):
        ...
        if len(var) >0:
            if " " in var:
                ...
                labels.append(var)
    labels = list(sorted(labels,key = lambda t:len(t),reverse= True))
    return labels
...
def transfer_train(train_file,transfer_save_file,label_files = "../res/parameter/labels.txt"):
    fe_size = fe.feature_size
...
def csv2arff(csv_file,save_path,class_index = -1):
    ...
    for i in range(1,len(content)):
        ...
        if i % int(len(content)/100) ==0:
            print(int(i/int(len(content)/100)),100)
